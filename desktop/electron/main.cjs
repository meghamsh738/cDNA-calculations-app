...
const getDefaultPaths = () => {
  const base = path.join(app.getPath('documents'), 'Easylab', 'cDNA Calculations')
  return {
...
const spawnBackend = async () => {
  ...
  dialog.showMessageBox({
    ...
    message: 'The cDNA backend could not start. Install Python 3.10+ or set APP_PYTHON_PATH, then restart.',
  })
}
